...
- Refactored the `cmd` layer (Phase 4-B): the three pre-removal safety checks (uncommitted / unpushed / merged) are now run by a single `runSafetyChecks` helper in `cmd/safety.go`; `gw end` and `gw clean` format its result into their own (intentionally distinct) wording. Broken-worktree detection for `gw clean` now branches on the Phase 3 `git.GitError` exit code (128) via `errors.As` instead of matching the `"exit status 128"` substring. Worktree path derivation (`<repoRoot>/../<repoName>-<suffix>`) is centralized in `git.ResolveWorktreePath` and reused by worktree creation/removal, `gw checkout`, and `gw shell-integration --print-path`. No user-facing behavior change.
- Refactored the `cmd` layer (Phase 4-C): the long `Execute()` methods of `gw start` / `gw checkout` / `gw end` / `gw clean` were decomposed into focused private methods (resolve → create → post-create for start/checkout; resolve → confirm → remove for end; a `checkWorktrees` helper for clean), preserving processing order and output. Each command now depends on a narrowed git interface (`startGit` / `checkoutGit` / `endGit` / `cleanGit`) covering only the operations it uses, `gw checkout` and `gw shell-integration --print-path` call `git.SanitizeBranchNameForDirectory` directly instead of via the interface, `defaultBaseBranch` moved to `cmd/command.go`, and `gw clean`'s `{main, master}` protection list is now a named `protectedBranches` constant. No user-facing behavior change.
