...
### Fixed
- `gw clean` no longer treats an "exit status 128" string match as a broken-worktree indicator; it now uses structured error inspection (`errors.As` on `git.GitError`), making the detection more robust and eliminating false positives from git error messages that happen to contain that substring.

### Internal
- Refactored the `internal/git` package: git subprocess execution is centralized in a single runner, and the former package-level git functions are methods on `*git.Client`. The 38 pass-through delegation methods and the `DefaultClient` struct were removed. No user-facing behavior change. (Phase 3-A)
- Refactored the `cmd` layer: configuration is loaded once in `DefaultDependencies` and injected via `Dependencies.Config` (guaranteed non-nil) rather than re-loaded per command. The eight `NewXCommandWithConfig` constructors and scattered `config != nil` guards were removed. No user-facing behavior change beyond the config-load warning in Changed above. (Phase 4-A)
- Refactored the `cmd` layer: the three pre-removal safety checks (uncommitted / unpushed / merged) are now performed by a single `runSafetyChecks` helper in `cmd/safety.go` shared by `gw end` and `gw clean`. Worktree path derivation is centralized in `git.ResolveWorktreePath`. No user-facing behavior change. (Phase 4-B)
- Refactored the `cmd` layer: the long `Execute()` methods of `gw start` / `gw checkout` / `gw end` / `gw clean` were decomposed into focused private methods, and each command now depends on a narrowed git interface covering only the operations it uses. No user-facing behavior change. (Phase 4-C)
- Eliminated the 5-fold repetition of config key definitions in `internal/config`: a `fieldSpecs` table is now the single source of truth for key names, types, defaults, and descriptions; `Load` / `Save` / `GetConfigItems` / `SetConfigItem` all walk the table. No user-facing behavior change. (Phase 5-1)
